...
## What actually moves the score (learned self-applying it on sibling servers)
...
- **Name the alternative tool.** "Use X instead of Y when Z" is the single
  ...
  cross-reference each other — e.g. `get_recommendation` (a single stored
  recommendation edge) vs `get_recommendations_for_media` (the full list for
  one title), or `search_activity` vs `get_user_activity` (one user's feed
...
- **Disclose the return shape, not just the auth/key caveat.** Behavioral
  ...
  credentials. This includes disclosing a hard-error consequence an
  annotation doesn't cover on its own (e.g. a tool annotated
  `idempotentHint: true` whose underlying mutation actually errors, rather
  than silently no-opping, when called again on an already-gone target —
  the annotation alone doesn't tell a caller that).
- **Parameter Semantics has a structural ceiling.** If a schema's
  ...
  genuinely non-obvious fact the schema can't express (e.g. `search_activity`'s
  `user` field noting it accepts an exact username with no fuzzy matching).
