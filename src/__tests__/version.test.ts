...
describe("renderChangelogRelease", () => {
  ...
  test("gets a heading with a placeholder note when [Unreleased] has no bullets (e.g. CONFIRM_EMPTY_CHANGELOG=1 release)", () => {
    const fixture = "## [Unreleased]\n\n## [0.9.0] - 2026-07-29\n\n### Added\n\n- Old thing.\n";
    const out = renderChangelogRelease(fixture, "0.10.0", "2026-08-01");

    assert.match(out, /## \[Unreleased\]\n\n## \[0\.10\.0\] - 2026-08-01\n/);
    assert.match(out, /_No user-facing changes in this release\._/);
    assert.match(out, /## \[0\.9\.0\] - 2026-07-29\n\n### Added\n\n- Old thing\./);
  });

  test("gets a heading with a placeholder note when [Unreleased] has only blank lines before the next heading", () => {
    const fixture = "## [Unreleased]\n\n\n## [0.9.0] - 2026-07-29\n\n- Old thing.\n";
    const out = renderChangelogRelease(fixture, "0.10.0", "2026-08-01");
  ...
  test("is idempotent: re-running once this version's heading already exists is a no-op", () => {
    const fixture = "## [Unreleased]\n\n### Fixed\n\n- Something.\n\n## [0.9.0] - 2026-07-29\n";
    const once = renderChangelogRelease(fixture, "0.10.0", "2026-08-01");
    const twice = renderChangelogRelease(once, "0.10.0", "2026-08-01");
...
describe("unreleasedHasBullets", () => {
  ...
  test("false when [Unreleased] has only blank lines before the next heading", () => {
    const fixture = "## [Unreleased]\n\n\n## [0.9.0] - 2026-07-29\n\n- Old thing.\n";
    assert.equal(unreleasedHasBullets(fixture), false);
  });
